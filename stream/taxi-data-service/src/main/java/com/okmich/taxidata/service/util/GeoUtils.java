 ...
 */
package com.okmich.taxidata.service.util;
...
public class GeoUtils implements Serializable {
    ...
    /**
     * Checks if a location specified by longitude and latitude values is within
     * the geo boundaries of New York City.
     *
     ...
     * grid covering the area of NYC. The grid cells are roughly 100 x 100 m and
     ...
     * latitude to a list of cells of a grid covering the area of NYC. The grid
     * cells are roughly 100 x 100 m and sequentially number from north-west to
     * south-east starting by zero.
     *
    ...
    public static float getGridCellCenterLon(int gridCellId) {
        ...
        return (float) (Math.abs(LON_WEST) - (xIndex * DELTA_LON) - (DELTA_LON / 2))
                * -1.0f;
    }
    ...
    /**
     * Returns the Euclidean distance between two locations specified as lon/lat
     * pairs.
     *
     ...
     */
    public static double getEuclideanDistance(float lon1, float lat1,
            float lon2, float lat2) {
        double x = lat1 - lat2;
     ...
     * destination and the x-axis on which the start is located.
     *
     * The angle describes in which direction the destination is located from
     * the start, i.e., 0° -> East, 90° -> South, 180° -> West, 270° -> North
     *
     ...
     */
    public static int getDirectionAngle(float startLon, float startLat,
            float destLon, float destLat) {
